...
class edition_Model extends MY_Model {
    ...
    public function validate_form($id=-1, $part=1){
        ...
        if($part == 1){
            ...
            $anz = count($this->Model->getRows('edition', 'edition', array('edition'=> $this->input->post('edition', true), 'edition_id !=' =>$id)));
            if($anz <1 && $id!=-1){
                $this->form_validation->set_rules('edition', '', 'required|trim|xss_clean');
            }
            else{
                $this->form_validation->set_rules('edition', '', 'required|trim|xss_clean|is_unique[edition.edition]');
            }
        }
        if($part == 2){
...
/* Location: ./buchverwaltung/application/models/redaktion/edit_model.php */
