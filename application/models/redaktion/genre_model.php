...
class Genre_Model extends MY_Model {
    ...
    public function validate_form($id=-1, $part=1){
        ...
        if($part == 1){
            ...
            $anz = count($this->Model->getRows('genre', 'genre', array('genre'=> $this->input->post('genre', true), 'genre_id !=' =>$id)));
            if($anz <1 && $id!=-1){
                $this->form_validation->set_rules('genre', '', 'required|trim|xss_clean');
            }
            else{
                $this->form_validation->set_rules('genre', '', 'required|trim|xss_clean|is_unique[themenkreis.themenkreis]');
            }
        }
        if($part == 2){
...
/* Location: ./buchverwaltung/application/models/redaktion/genre_model.php */
