...
class Genre extends Adminpage_controller {

    protected $sControllerName = '';
    protected $sTable = 'genre';
    protected $sUriUebersicht = 'redaktion/genreuebersicht/';
    ...
    public function index () {
        ...
        if ($this->input->post()) {
            ...
            //Falls Validierung korrekt, wird der Eintrag in die DB geschrieben
                ...
                // Daten auslesen
                $aData = array('genre' => $this->input->post('genre', true));

                // Datensatz eintragen
                if ($genre_id = $this->Model->saveRecord($this->sTable, $aData)) {

                    // wenn Buch-Daten in der Session sind diese ergänzen und zurückleiten
                    if($this->session->userdata('buchdata')){
                        $buchdata               = $this->session->userdata('buchdata');
                        $buchdata['genre_id']   = $genre_id;
                        $this->session->set_userdata('buchdata', $buchdata);
                        $buchdata = $this->session->userdata('buchdata');
                        redirect(base_url() . $buchdata['url']);
                    }
                    else{
                        redirect(base_url() . 'redaktion/genreuebersicht');
                    }
    ...
    public function edit ($id = '', $current_page = '', $account = 'edit') {
        ...
        if ($id != '') {
            ...
            if ($this->input->post('genre', true) && $this->Model->validate_form($id) != false) {

                    $aData = array('genre' => $this->input->post('genre', true));
                    $this->Model->saveRecord($this->sTable, $aData, $id, 'genre_id');
...
/* Location: ./buchverwaltung/application/controllers/redaktion/genre.php */